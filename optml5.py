...
dataset_portions = [1e-3, 5e-3]     # Portions of complete dataset for the accuracy vs dataset size
...
regu = [0, 5e-4, 5e-3, 5e-2, 5e-1] [0:2]            # or [-1:] for last element
                    ...
                    for subset_size in dataset_sizes:
                        ...
                        for epoch in tqdm(range(num_epochs), desc=f'Training with dataset size {subset_size}'):
                            ...
                            for images, labels in train_loader[dataset_sizes.index(subset_size)]:
                                ...
                                abs_weight_matrix = torch.abs(model.classifier[3].weight)
                                #print(abs_weight_matrix.requires_grad)
                                l1_loss = reg * torch.sum(abs_weight_matrix)
                                l2_loss = reg * matrix_square_sum(abs_weight_matrix)
        ...
        for lr in learning_rate:
            for reg in regu:
                ...
                if acc_mean_per_subset:
                    ...
                    plt.errorbar(dataset_sizes, acc_mean_per_subset, yerr=acc_std_per_subset, fmt=marker_style, linestyle=line_style, capsize=5, capthick=2, label=f'{model_name} {opt} {lr} {reg} Accuracy')
                    ...
                    plt.errorbar(dataset_sizes, rec_mean_per_subset, yerr=rec_std_per_subset, fmt=marker_style, linestyle=line_style, capsize=5, capthick=2, label=f'{model_name} {opt} {lr} {reg} Recall')
