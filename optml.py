...
if model == "scatterCNN":
    model = ScatteringModel(J, L, order, input_shape=(1, 28, 28), device=device); print("Scattering model selected")
elif model == "normalCNN":
    model = NormalModel(input_shape=(1, 28, 28)); print("Normal model selected")
else:
...
accuracies = [[] for i in range (num_experiments)]
history = {}
for subset_size in dataset_sizes:
    history["{}_loss".format(subset_size)] = [[] for i in range(num_experiments)]
    history["{}_val_loss".format(subset_size)] = [[] for i in range(num_experiments)]
...
for experiment in range(num_experiments):
    ...
    for subset_size in dataset_sizes:
        ...
        for epoch in tqdm(range(num_epochs), desc=f'Training with dataset size {subset_size}'):
            ...
            history['{}_loss'.format(subset_size)][experiment].append(total_loss / len(train_subset))
            ...
            history['{}_val_loss'.format(subset_size)][experiment].append(val_loss / len(val_subset))
...
probabilities = torch.softmax(outputs,
                              dim=1).detach().cpu().numpy()  # Detach and then convert softmax probabilities to CPU and numpy array
...
## The metrics are for the last experiment
...
for subset_size in dataset_sizes:
    ...
    loss_mean, loss_error = process_data(np.array(history['{}_loss'.format(subset_size)]))
    val_loss_mean, val_loss_error = process_data(np.array(history['{}_val_loss'.format(subset_size)]))
    plt.figure(figsize=(10, 6))
    plt.errorbar(epochs, loss_mean, yerr=loss_error, fmt='-o', capsize=5, capthick=2, label='Train Loss')
    plt.errorbar(epochs, val_loss_mean, yerr=val_loss_error, fmt='-o', capsize=5, capthick=2, label='Validation Loss')
    plt.title('Model loss for dataset size {}'.format(subset_size))
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.show()

# Plot validation accuracy values vs dataset size
acc_mean, acc_error = process_data(np.array(accuracies))
plt.figure(figsize=(10, 6))
plt.errorbar(dataset_sizes, acc_mean, yerr=acc_error, fmt='-o', capsize=5, capthick=2)
