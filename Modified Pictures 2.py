...
dataset_portions = [0.0001, 0.0005, 0.003]  # Portions of complete dataset for the accuracy vs dataset size
...
learning_rate = [1e-1, 1e-2, 1e-3][1:3]  # e.g. [1:2] for second element
            ...
            for reg in regu:
                ...
                for experiment in range(num_experiments):
                    ...
                    for subset_size in dataset_sizes:
                        ...
                        auc = roc_auc_score(test_labels, outputs.cpu().detach().numpy(),
                                            multi_class="ovr")
    ...
    for opt in optimizer_list:
        for lr in learning_rate:
            for reg in regu:
                for subset_size in dataset_sizes:
                    ...
                    if loss_key in history[model_name] and acc_key in accuracies[model_name]:
                        ...
                        if loss_data.size and val_loss_data.size and acc_data.size:
                            ...
                            # axs[i].errorbar(epochs, loss_mean, yerr=loss_error, fmt='-',
                            #                 label=f'{model_name} {opt} {lr} {reg} Train Loss')
                            # axs[i].errorbar(epochs, val_loss_mean, yerr=val_loss_error, fmt='--',
                            ...
                            plt.yscale('log')
                            plt.legend()
                            plt.savefig("{}_loss.png".format(config_key))
                            plt.clf()
                ...
                plt.ylabel('Accuracy')
                plt.legend()
                plt.savefig("{}_accuracy.png".format(config_key))
                plt.clf()
...
# ax.errorbar(dataset_sizes, acc_mean_per_subset, yerr=acc_std_per_subset, fmt='o-', capsize=5, capthick=2,
#             label=f'{model_name} {opt} {lr} {reg} Accuracy')
